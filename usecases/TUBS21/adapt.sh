...
run_adapt110() {
	...
	cmd="./mcc_tubs.py --adapt --basepath \"${BASEPATH}\" --outpath \"${OUTPATH}\" \"$exp\" 2>&1"
	echo "Running $cmd"
...
run_adapt200() {
	...
	cmd="./mcc_tubs.py --adapt --wcet_factor 2.0 --basepath \"${BASEPATH}\" --outpath \"${OUTPATH}\" \"$exp\" 2>&1"
	echo "Running $cmd"
	unbuffer sh -c "$cmd" > "${OUTPATH}output.log"
	succ=$(cat "${OUTPATH}output.log" | grep 'Stats' | wc -l)
	if [ 2 -eq $succ ] ; then
...
run_adapt150() {
	...
	cmd="./mcc_tubs.py --adapt --wcet_factor 1.5 --basepath \"${BASEPATH}\" --outpath \"${OUTPATH}\" \"$exp\" 2>&1"
	echo "Running $cmd"
	unbuffer sh -c "$cmd" > "${OUTPATH}output.log"
	succ=$(cat "${OUTPATH}output.log" | grep 'Stats' | wc -l)
	if [ 2 -eq $succ ] ; then
